...
try:
    response = es.search_template(
        index="ton_index",  # Remplace 'ton_index' par le nom de ton index
        body={
    ...
    # Extraire 'took' et 'value' de la réponse
    took = response.get('took')
    value = response.get('aggregations', {}).get('countServedIsdn', {}).get('value')

    # Afficher uniquement 'took' et 'value'
    print(f"'took': {took}, 'value': {value}")
